...
def on_interest_i4(name, param, app_param):
    ...
    print(f"[Consumer] Received Interest I_4: {Name.to_str(name)}")
    try:
        ...
        print(f"[Consumer] Sent Data D_4: {Name.to_str(name)}")
...
async def start_upload(gateway_prefix, producer_prefix, session_id, chunk_size):
    ...
    try:
        print(f"[Consumer] Sending I_1 to Gateway for Producer '{producer_prefix}', session {session_id}")
        metrics["tx_i"] += 1  # 送信Interest(I_1)
        print(f"[Consumer] Sent Interest I_1: {name}")
        ...
        print(f"[Consumer] Received Data D_1: {Name.to_str(data_name)}")
